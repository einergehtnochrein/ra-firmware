...
/* RS92 raw frame */
static uint8_t _rs92_raw[234] = {
    
};
...
static uint8_t _rs92_null;

static uint8_t * _rs92_getDataAddress (int index)
{
    if (index < 210) {
        return &_rs92_raw[index];
    }
    else if (index >= 231) {
        return &_rs92_raw[index - 21];
    }
    else {
        _rs92_null = 0;
        return &_rs92_null;
...
/* Test of the "general" Reed Solomon decoder, but uses RS92 frame as an example */
TEST(fec, reedsolomon_rs92)
{
    int numErrors;
    int i;
    ...
    /* Begin with a correct codeword. Should be accepted, and no errors detected. */
    ENUMS_EQUAL_INT(LPCLIB_SUCCESS, REEDSOLOMON_process(24, 0, 1, 1, _rs92_getDataAddress, &numErrors));
    LONGS_EQUAL_TEXT(0, numErrors, "No corrections in valid CW");

    /* Add some errors, max. 12. This is still correctable. */
    int errorLocations[] = {0, 23, 62, 64, 66, 110, 120, 130, 161, 177, 215, 233}; /* 215,233 in parity block */
    int nErrors = sizeof(errorLocations) / sizeof(errorLocations[0]);
    for (i = 0; i < nErrors; i++) {
        _rs92_raw[errorLocations[i]] ^= 0x42;
    }
