...
RTOS_INCLUDE= \
    ...
    -I$(PDL)/libs/rtos/$(RTOS)/src/portable/GCC/ARM_CM0
RTOS_SOURCEDIR= \
    $(PDL)/libs/rtos/$(RTOS) \
    $(PDL)/libs/rtos/$(RTOS)/src \
    $(PDL)/libs/rtos/$(RTOS)/src/portable/MemMang \
    $(PDL)/libs/rtos/$(RTOS)/src/portable/GCC/ARM_CM0
RTOS_SRCS= \
...
RTOS_SRCS= \
    RTX_Conf_CM.c
RTOS_CFLAGS= -DRTX
RTOS_LDFLAGS= -Wl,-L$(PDL)/libs/rtos/$(RTOS)/GCC -Wl,-lRTX_CM0
endif
...
COMMONFLAGS += \
    -DLPCLIB_FAMILY=LPCLIB_FAMILY_LPC5411X -DCORE_M0 \
...
# Sections to keep for M0 binary image
M0SECTIONS = -j .text -j .bootstrap -j .data1 -j .ramcode -j .scattertable
...
$(OBJDIR)/$(PROJECT).hex : $(MAKEFILE) $(OBJDIR)/$(PROJECT).axf
	...
	@$(OBJCOPY) $(OBJDIR)/$(PROJECT).axf -O binary $(M0SECTIONS) $(OBJDIR)/$(PROJECT).bin
